...
## v0.24.0 — fork manual operation (2026-09-07)

- PCの個別チャットで引用返信、送信担当者の表示、受信動画・音声・ファイルの確認が可能に。
- 手動運用モードに受信キュー、担当者ロック、送信結果が不確かな場合の二重送信防止を追加。
- 取得できなかった表示名や添付を後から復旧し、開いている会話にも反映。
- 大きな添付を分割保存し、過去履歴の保持時に引用・担当者情報も保存。
- 新規環境のDB初期化と専用リソース設定を修正。
